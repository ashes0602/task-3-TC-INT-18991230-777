...
const Services = () => {
  ...
  const fetchServices = async (paramsString = '') => {
    setLoading(true);
    try {
      const { data } = await axios.get(`http://localhost:5000/api/services${paramsString}`);
      setServices(data);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchServices(queryString ? `?${queryString}` : '');
  }, [searchParams]);
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center mb-6">
        ...
        {user && (
          <Link to="/services/new" className="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 transition">
            Create Service
          </Link>
        )}
      </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.map((service) => (
                <div key={service._id} className="bg-white shadow rounded-lg overflow-hidden border border-gray-100 hover:shadow-lg transition flex flex-col">
                  <img 
                    src={service.portfolioImages[0] || 'https://via.placeholder.com/300x200?text=No+Image'} 
                    alt={service.title} 
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto pt-4">
                      ...
                      <Link 
                        ...
                        className="block w-full text-center bg-indigo-50 text-indigo-700 py-2 rounded-md font-medium hover:bg-indigo-100 transition"
                      >
                        View Details
                      </Link>
                    </div>
              ...
              {services.length === 0 && (
                <div className="col-span-full text-center text-gray-500 py-10">No services found matching your criteria.</div>
              )}
            </div>
          )}
        </div>
